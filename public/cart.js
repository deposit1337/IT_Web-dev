...
function updateOrderInfo(order) {
    ...
    orderFormBtn.disabled = orderSize === 0;
...
function onAmountControlBtnClick(click) {
    ...
    if (btn.classList.contains("amount-control__minus-btn")) {
        pizzaInOrder.amount--;
        btn.disabled = pizzaInOrder.amount < 2;
    } else {
        pizzaInOrder.amount++;
...
function createOrderItem(orderItemObj) {
    ...
    minusBtn.disabled = orderItemObj.amount < 2;
...
async function onOrderFormSubmit(submit) {
    ...
    const response = await fetch(orderForm.action, {
        ...
        body: JSON.stringify({ "customer_name": customerName.value, "address": address.value, "pizzas": order })
    });
